...
def build_model(encoders):
    ...
    hidden = Dense(64, activation='selu', name='hidden_1',
...
def build_encoders(df):
    ...
    # make
    make_enc = df['make']
    make_bins = make_enc.quantile(np.linspace(0, 1, 4+1))
    ...
    body_perc = max(floor(0.1 * body_counts.size), 1)
    ...
    mileage_bins = mileage_enc.quantile(np.linspace(0, 1, 4+1))
    ...
    # engV
    engv_enc = df['engV']
    engv_bins = engv_enc.quantile(np.linspace(0, 1, 4+1))
    ...
    # engType
    engtype_counts = df['engType'].value_counts()
    engtype_perc = max(floor(0.1 * engtype_counts.size), 1)
    engtype_top = np.array(engtype_counts.index[0:engtype_perc], dtype=object)
    engtype_encoder = LabelBinarizer()
    engtype_encoder.fit(engtype_top)
    ...
    registration_perc = max(floor(0.1 * registration_counts.size), 1)
    ...
    # year
    year_enc = df['year']
    year_bins = year_enc.quantile(np.linspace(0, 1, 4+1))
    ...
    drive_perc = max(floor(0.1 * drive_counts.size), 1)
