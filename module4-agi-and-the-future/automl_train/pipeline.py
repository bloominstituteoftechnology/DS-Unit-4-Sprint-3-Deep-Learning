...
def build_model(encoders):
    ...
    # Age
    input_age = Input(shape=(4,), name="input_age")
    ...
    # Fare
    input_fare = Input(shape=(4,), name="input_fare")
    ...
    for i in range(6-1):
        hidden = Dense(256, activation="selu", name="hidden_{}".format(
    ...
    output = Dense(1, activation="sigmoid", name="output",
                   kernel_regularizer=l2(1e-3))(hidden)
...
def build_encoders(df):
    ...
    # Age
    age_enc = df['Age']
    age_bins = age_enc.quantile(np.linspace(0, 1, 4+1))
    ...
    # Siblings/Spouses Aboard
    siblings_spouses_aboard_counts = df['Siblings/Spouses Aboard'].value_counts()
    siblings_spouses_aboard_perc = max(
        floor(0.5 * siblings_spouses_aboard_counts.size), 1)
    siblings_spouses_aboard_top = np.array(
        siblings_spouses_aboard_counts.index[0:siblings_spouses_aboard_perc], dtype=object)
    siblings_spouses_aboard_encoder = LabelBinarizer()
    siblings_spouses_aboard_encoder.fit(siblings_spouses_aboard_top)
    ...
    # Parents/Children Aboard
    parents_children_aboard_counts = df['Parents/Children Aboard'].value_counts()
    parents_children_aboard_perc = max(
        floor(0.5 * parents_children_aboard_counts.size), 1)
    parents_children_aboard_top = np.array(
        parents_children_aboard_counts.index[0:parents_children_aboard_perc], dtype=object)
    parents_children_aboard_encoder = LabelBinarizer()
    parents_children_aboard_encoder.fit(parents_children_aboard_top)
    ...
    # Fare
    fare_enc = df['Fare']
    fare_bins = fare_enc.quantile(np.linspace(0, 1, 4+1))
...
def load_encoders():
    ...
    age_encoder.classes_ = list(range(4))
    ...
    fare_encoder.classes_ = list(range(4))
